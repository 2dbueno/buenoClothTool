                    ...
                    if ((geom.Shader != null) && (geom.Shader.ParametersList != null) && (geom.Shader.ParametersList.Hashes != null))
                    {
                        ...
                        for (int ip = 0; ip < h.Length; ip++)
                        {
                            ...
                            if (tex != null)
                            {
                                ...
                                // PROTEÇÃO: Verificamos se Name é nulo. Se for, usamos "Unknown"
                                string safeName = string.IsNullOrEmpty(tex.Name) ? "Unknown_Texture" : tex.Name.Trim();

                                var tstr = safeName;

                                if (t != null)
                                {
                                    // Usamos safeName aqui também para evitar erro no string.Format
                                    tstr = string.Format("{0} ({1}x{2}, embedded)", safeName, t.Width, t.Height);
                                }
