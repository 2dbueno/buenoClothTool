...
namespace buenoClothTool.Views
{
    public partial class DuplicateInspectorWindow : Window
    {
        ...
        private void IgnoreGroup_Click(object sender, RoutedEventArgs e)
        {
            if (sender is System.Windows.Controls.Button button && button.Tag is DuplicateGroupViewModel group)
            {
                var result = Controls.CustomMessageBox.Show(
                    $"Are you sure you want to whitelist this group?\n\nThese {group.Count} items will no longer be marked as duplicates in this project.",
                    "Ignore Duplicates",
                ...
                if (result == Controls.CustomMessageBox.CustomMessageBoxResult.Yes)
                {
                    ...
                    DuplicateDetector.IgnoreGroup(group.GroupId);

                    // Salva o projeto para persistir a whitelist
                    SaveHelper.SetUnsavedChanges(true);

                    // Recarrega a janela para o grupo sumir da lista
                    LoadAllDuplicates();
        ...
        {
            var window = new IgnoredListWindow
            {
                Owner = this
            };
            window.ShowDialog();

            // Quando a janela fechar, recarregamos as duplicatas para refletir as mudanças
            // (Caso o usuário tenha removido algo da whitelist, ele deve reaparecer aqui se rodarmos o scan de novo)
            // Nota: Para reaparecer imediatamente, teríamos que forçar um DuplicateDetector.RescanDrawables(),
            // mas apenas LoadAllDuplicates() vai recarregar o que já está na memória.
            // O ideal é pedir um Refresh manual ou forçar aqui:

            DuplicateDetector.RescanDrawables(); // Força re-verificar tudo
            LoadAllDuplicates(); // Atualiza a tela
        }
