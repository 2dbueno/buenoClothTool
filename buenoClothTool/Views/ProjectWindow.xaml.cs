    ...
    public partial class ProjectWindow : UserControl, INotifyPropertyChanged
    {
        ...
        private void SelectedDrawable_Changed(object sender, EventArgs e)
        {
            ...
            if (Addon.SelectedDrawables.Count == 1)
            {
                ...
                // ADIÇÃO: LAZY LOADING
                // Carregamos APENAS as texturas do item clicado.
                if (Addon.SelectedDrawable.Textures != null)
                {
                    foreach (var texture in Addon.SelectedDrawable.Textures)
                    {
                        _ = texture.EnsureDetailsLoaded();
                    }
                }
