...
public static class DuplicateDetector
{
    ...
    public static void RegisterDrawable(GDrawable drawable)
    {
        ...
        if (MainWindow.AddonManager.IgnoredDuplicateGroups.Contains(hash))
        {
            return; // Se estiver ignorado, não registra como duplicata
        }
    ...
    public static void IgnoreGroup(string hash)
    {
        if (string.IsNullOrEmpty(hash)) return;

        // Adiciona à lista de ignorados do projeto (para salvar no JSON)
        if (!MainWindow.AddonManager.IgnoredDuplicateGroups.Contains(hash))
        {
            MainWindow.AddonManager.IgnoredDuplicateGroups.Add(hash);
        }

        // Limpa as marcações visuais dos itens desse grupo
        if (_drawableDuplicateGroups.TryGetValue(hash, out var group))
        {
            ...
            // Remove da lista interna de detecção
