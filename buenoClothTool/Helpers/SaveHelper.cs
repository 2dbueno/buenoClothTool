...
public static class SaveHelper
{
    ...
    public static async Task SaveAsync()
    {
        ...
        try
        {
            ...
            // Inicializa a variável json fora do escopo da thread
            string json = "";

            // Forçamos a serialização a ocorrer na Thread Principal (UI).
            // Isso impede que o user delete algo ENQUANTO o json está sendo criado.
            ...
            try
            {
                    ...
                    Directory.Exists(mainProjectsFolder))
                {
                    ...
                    // A escrita no disco continua sendo assíncrona para não travar o PC
            ...
            // Invoke na UI Thread também para eventos de UI, por segurança
            System.Windows.Application.Current.Dispatcher.Invoke(() =>
            {
                SaveCreated?.Invoke();
        ...
        {
            LogHelper.Log($"Critical error in SaveAsync: {ex.Message}", Views.LogType.Error);
        }
        finally
        {
            _semaphore.Release();
    ...
    public static async Task LoadSaveFileAsync(string filePath)
    {
        try
        {
            ...
            // Recupera a lista de ignorados do JSON para a memória
            MainWindow.AddonManager.IgnoredDuplicateGroups.Clear();
            if (addonManager.IgnoredDuplicateGroups != null)
            {
                foreach (var hash in addonManager.IgnoredDuplicateGroups)
                {
                    MainWindow.AddonManager.IgnoredDuplicateGroups.Add(hash);
                }
            }
